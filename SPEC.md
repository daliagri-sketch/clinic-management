...
## דף פגישות לפי מטופל (`/patients/[id]`)

מסך חדש שמציג את כל הפגישות של מטופל ספציפי, לפי חודש.

- כניסה דרך לחצן **"פגישות"** בכל שורה ברשימת המטופלים.
- ניווט חודשי (חודש קודם / הבא), ברירת מחדל = החודש הנוכחי.
- כותרת עם שם המטופל, icount\_id, תעריף וטלפון.
- אירועי היומן מסוננים לפי `calendar_aliases` של המטופל בלבד (לא כלל האירועים).
- לכל שורה: תאריך, שעה, תיאור ביומן, סטטוס פגישה, תשלום, אופן תשלום, חשבונית.
- פעולות זהות למסך ההתאמה: שמירת סשן, עדכון סטטוס/תשלום, הנפקת חשבונית מ-iCount.
- סשנים שנשמרו ידנית ואינם מופיעים עוד ביומן מוצגים גם הם.
- מטופלים ותיקים — מופיעים ב-iCount מראש.
- מטופלים חדשים — נוצרים ב-iCount אוטומטית עם הנפקת החשבונית הראשונה.
